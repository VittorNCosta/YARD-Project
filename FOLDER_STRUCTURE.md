...
├── database/               # Clientes e gestão de banco de dados
│   ├── database-client.ts  # Interface/abstração do cliente de banco
│   ├── providers/          # Providers de banco de dados
│   │   └── database.provider.ts
│   ├── redis/              # Cliente Redis (quando aplicável)
│   │   ├── redis-connection.ts
│   │   └── redis-session-store.ts
│   └── mongo/              # Cliente MongoDB (Mongoose)
│       ├── mongo-connection.ts   # Conexão Mongoose
│       └── utils/          # Utilitários MongoDB
...
└── infra/
    ...
    └── <entidade>/
        ...
        │   └── patch/
        ├── database/
        │   ├── repositories/  # Implementações concretas (ex.: MongoXXXRepository)
        │   │   └── mongo-<entidade>-repository.ts
        │   └── mappers/       # Domain <-> persistência
        │       └── mongo-<entidade>-mapper.ts
        ├── http/
...
└── infra/location/
    ...
    ├── database/
    │   ├── mappers/
    │   │   └── mongo-location-mapper.ts
    │   └── repositories/
    │       └── mongo-location-repository.ts
    ├── http/
    │   └── routes/
    │       └── location.routes.ts
    ├── presenter/
...
- Crie `src/modules/<modulo>/infra/<entidade>/database/`:
  - `schemas/<entidade>.schema.ts` - Schema Mongoose
  - `repositories/mongo-<entidade>-repository.ts` - Implementação do repositório
  - `mappers/mongo-<entidade>-mapper.ts` - Conversão domain ↔ documento Mongo
